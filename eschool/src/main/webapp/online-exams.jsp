...
<%@ page import="eschool.exam.service.Service"%>
<%@ page import="eschool.exam.model.Exam"%>
<%
			...
			<div class="modal-content">
				...
				<form method="POST" action="UpdateExam">
							...
							<div class="row">
								<div class="col-sm">
									<div class="form-group">
										<label for="formGroupExampleInput">Date time</label> <input
											type="datetime-local" class="form-control" id="formGroupExampleInput"
											placeholder="Name" name="datetime" step="1" required>
									</div>
									<div class="form-group">
										<label for="formGroupExampleInput">Duration</label> <input
											type="number" class="form-control" id="formGroupExampleInput"
											placeholder="Name" name="duration" required>
									</div>
								</div>
							</div>
						</div>
					...
					<div class="modal-footer">
					    <input type="hidden" value="" name="olddatetime">
                ...
                <section class="breadcumb-area card bg-gradient mb-5">
                    <div class="bread-cumb-content card-body d-flex align-items-center">
                        <div class="breadcumb-heading">
                            <h2 class="text-white">All Exams</h2>
                        </div>
                ...
                <section class="es-form-area">
                    <div class="card">
                        <header class="card-header bg-gradient border-0 pt-5 pb-5 d-flex align-items-center">
                            <a href="online-exam-add.jsp" class="btn btn-sm btn-pill btn-outline-light ml-auto">+ Add New</a>
                        </header>
                        <div class="card-body">
                                    ...
                                    <table class="table mb-0">
                                        <thead class="bg-gradient">
                                            <tr>
                                                ...
                                                <th scope="col" class="text-white text-center">Date time</th>
                                                <th scope="col" class="text-white text-center">Duration</th>
                                        ...
                                        <tbody>
                                        <%
											if (request.getAttribute("searchExamsByClassSectionSubject") != null) {
												ArrayList<Exam> result = (ArrayList<Exam>) request.getAttribute("searchExamsByClassSectionSubject");
												int count = 1;
												for (Exam e : result) {
											%>
											<tr>
												...
												<td><%=e.getDateTime()%></td>
												<td><%=e.getDuration()%></td>
												<td><form method="POST" action="DeleteExam">
														<button type="submit" class="btn btn-primary">Delete</button>
														<input type="hidden" value=<%=e.getDateTime().toString().replaceAll("\\s+","T")%>
															name="datetime">
															<input type="hidden" value=<%=e.getYear()%>
															name="class">
															<input type="hidden" value=<%=e.getSection()%>
															name="section">
															<input type="hidden" value=<%=e.getSubject()%>
															name="subject">
													</form></td>
												<td><a href="#exampleModalCenter" data-toggle="modal"
													data-target="#exampleModalCenter"
													data-exam-datetime=<%=e.getDateTime().toString().replaceAll("\\s+","T")%>
													data-exam-duration=<%=e.getDuration()%>
													data-exam-subject=<%=e.getSubject()%>
													data-exam-class=<%=e.getYear()%>
													data-exam-section=<%=e.getSection()%>
											...
											count++;
												...
												ArrayList<Exam> result = Service.getExams();
												int count = 1;
												for (Exam e : result) {
											%>
											<tr>
		...
		$('#exampleModalCenter')
				.on(
						...
						function(e) {
							...
							var datetime = $(e.relatedTarget).data('exam-datetime');
							var year = $(e.relatedTarget).data('exam-class');
							var section = $(e.relatedTarget).data(
									'exam-section');

							var subject = $(e.relatedTarget).data(
							'exam-subject');
							var duration = $(e.relatedTarget).data(
							'exam-duration');
							console.log(datetime);
							//populate the textbox
							$(e.currentTarget)
									.find('input[name="datetime"]').val(
											datetime);
							$(e.currentTarget)
							.find('input[name="olddatetime"]').val(
									datetime);
							$(e.currentTarget)
							...
							$(e.currentTarget).find('input[name="subject"]').val(
									subject);
							$(e.currentTarget).find('input[name="duration"]')
									.val(duration);
						});
	</script>
</body>
